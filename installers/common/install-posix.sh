...
MSG_ERR_TMPDIR='Каталог временных файлов недоступен для записи: %s (задайте TMPDIR существующим каталогом)'
...
# ревизии 1.11, поля не содержат, а неподписанный бандл без снятия карантина не запускается вовсе.
...
json_flatten() {
  awk '
    ...
    function emit(k, v) {
      # Последняя страховка транспорта: строковые значения управляющих символов уже не содержат
      # (их снял pstring), но если сюда всё же попадёт разделитель формата, он заменяется тем же
      # маркером ctl, а НЕ пробелом: пробел — разрешённый белым списком байт (N5-P4), и подмена
      # им превращала бы запрещённое значение в допустимое (BUG-I5-003).
      gsub(/[\n\r\t]/, ctl, k)
      gsub(/[\n\r\t]/, ctl, v)
      print k "\t" v
    }
    # Управляющий символ в строке не восстанавливается побайтово: промежуточный формат KV
    # разделён табуляцией и переводом строки, и сырой байт сломал бы транспорт. Но и заменять
    # его ПРОБЕЛОМ, как делала ревизия до BUG-I5-003, нельзя: пробел белым списком N5-P4 внутри
    # app_name РАЗРЕШЁН («OpenCode Magnit.app»), поэтому такая подмена отмывала бы запрещённый
    # байт в разрешённый ещё до проверки — манифест с "a\tb.app" принимался кодом 0, а install.ps1
    # (строгий ConvertFrom-Json) тот же пакет отвергал Test-AppName кодом 2.
    # Решение вынесено сюда, а не в is_safe_pkg_path/is_safe_app_name, потому что проверки видят
    # только результат разбора: к ним значение приходит уже без управляющего символа, и никакой
    # набор правил там его не восстановит. Разбор при этом не отвергает документ сам (это дало бы
    # общее «манифест невалиден» без имени поля) — он подставляет ctl, байт, запрещённый во ВСЕХ
    # путевых полях. Так вердикт по-прежнему выносит белый список — единственное место, где задан
    # допустимый набор символов, — и сообщение называет поле (AC-145, N5-P3, N5-P4).
    # \uXXXX разбирается по кодовой точке, а не глушится целиком: install.ps1 берёт
    # ConvertFrom-Json, который "\u0074ander-ca-bundle.pem" декодирует в "tander-ca-bundle.pem"
    # и пакет принимает, а POSIX-парсер подставлял маркер и тот же пакет отвергал кодом 2 —
    # прямое нарушение паритета платформ (spec.md N5-P4: набор принятых манифестов одинаков).
    # Декодируется только печатный ASCII 0x20-0x7e — ровно то, что вообще может оказаться
    # разрешённым белым списком; управляющие символы (включая \u0000-\u001f и 0x7f) и любые
    # не-ASCII кодовые точки по-прежнему становятся ctl, то есть байтом, запрещённым во всех
    # путевых полях. Отмывки запрещённого в разрешённое не возникает: декодированный символ
    # идёт в белый список как есть, поэтому "\u0024" остаётся отвергнутым "$", а не маркером.
    function hex4(s,   i, c, d, n) {
    ...
    function pstring(   res, c, code) {
      ...
      while (pos <= dlen) {
        ...
        if (c == "\\") {
          pos++
          c = substr(doc, pos, 1)
          if (c == "n" || c == "t" || c == "r" || c == "b" || c == "f") { res = res ctl }
          else if (c == "u") {
            code = hex4(substr(doc, pos + 1, 4))
            if (code >= 32 && code <= 126) { res = res sprintf("%c", code) } else { res = res ctl }
            pos = pos + 4
          }
          else { res = res c }
          pos++
          ...
          # Сырой (неэкранированный) разделитель внутри строки: RFC 8259 такие байты запрещает,
          # но парсер их принимал, а emit глушил пробелом — та же отмывка, что и выше. Прочие
          # управляющие байты (0x01-0x08, 0x0b, 0x0c, 0x0e-0x1f, 0x7f) транспорт переживают и
          # доходят до белого списка как есть, поэтому подмены не требуют.
          if (c == "\t" || c == "\n" || c == "\r") { res = res ctl } else { res = res c }
          pos++
        }
      }
    ...
    { doc = doc $0 "\n" }
    END {
      # Маркер управляющего символа: печатный байт, которого нет в белом списке ни у пути внутри
      # пакета, ни у имени приложения, и который переживает KV-транспорт. См. pstring.
      ctl = "?"
      dlen = length(doc)
      pos = 1
      fail = ""
...
manifest_load() {
  ...
  # Адрес корпоративной точки входа (S-C10, ревизия 1.11). Hub перестал быть единственным:
...
# Пригодность каталога временных файлов проверяется в предусловиях, а не в момент mktemp:
# иначе установка обрывается посередине (CA уже скопирован, бинарник уже установлен) системной
# английской диагностикой mktemp и кодом 1 без объяснения, что установка частична (N5-I1).
require_tmpdir() {
  local dir=${TMPDIR:-/tmp}
  if [ ! -d "$dir" ] || [ ! -w "$dir" ]; then
    die "$EX_FAIL" "$MSG_ERR_TMPDIR" "$dir"
  fi
}
...
handle_user_config() {
  ...
  if [ -f "$cfg" ]; then
    if [ -f "$bak" ]; then
      ...
      cp "$cfg" "$bak"
      # Резервная копия лежит в каталоге конфига пользователя и создана установщиком, значит
      # принадлежит исходному пользователю (N5-I14). Без этого при --system она остаётся
      # root-овой: перезаписать или удалить её пользователь не может, а следующий запуск уходит
      # в ветку «резервная копия сохранена» и root-овый файл остаётся навсегда.
      chown_user_path "$bak"
...
main() {
  ...
  resolve_source_user
  compute_layout
  # Пригодность TMPDIR — предусловие: mktemp используется на шаге профиля, то есть уже после
  # установки CA и бинарника. Режимы --check и --dry-run временных файлов не создают.
  if [ "$opt_check" -eq 0 ] && [ "$opt_dry_run" -eq 0 ]; then
    require_tmpdir
  fi
