...
# Совместимость: bash 3.2 (N5-T5) — без declare -A, mapfile/readarray, ${var^^}, local -n, &>>.
  ...
  local dest=$1 with_path=$2
  {
    ...
    if [ "$profile_is_fish" -eq 1 ]; then
      ...
      if [ "$with_path" -eq 1 ]; then
        ...
        # Обоснование: строка попадает в файл профиля дословно — $PATH раскрывает shell пользователя.
