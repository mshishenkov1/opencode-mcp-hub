...
is_root() {
  # id берётся по абсолютному пути: в части тестов id подменён ловушкой в PATH, а вопрос
  # «мы правда под root?» относится к настоящему процессу, а не к ловушке.
  local id_bin
  id_bin=$(PATH="/usr/bin:/bin:/usr/sbin:/sbin" command -v id 2>/dev/null || printf 'id')
  [ "$("$id_bin" -u)" -eq 0 ]
}
