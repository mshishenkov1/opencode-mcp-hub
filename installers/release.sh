...
# S-B17 (AC-271 форковой спеки): «Desktop не собирали» и «собрали, но не для этой архитектуры» —
# разные вещи, и второе
...
find_desktop_artifact() {
  ...
  if [ "$os" = "darwin" ]; then
    ...
    # см. list_other_arch_dmgs и строку отчёта в build_target (S-B17).
...
build_target() {
  ...
  if desktop_src=$(find_desktop_artifact "$os" "$arch"); then
    ...
    # установщика «Desktop: не входит в пакет» уже у пользователя (S-B17).
    if [ "$os" = "darwin" ]; then
