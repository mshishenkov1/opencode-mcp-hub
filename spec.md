...
> **всего блока** — половинчатая публикация запрещена (R-U8.1 п. 6–8, решение 121). Исключение —
> четыре адреса (`upstream.url`, `verify.url`, `exchange.url`, `exchange.revoke.url`): без
> подставленного адреса прямого режима не существует, а `env:VAR` в адресе запрещён схемой
> (решение 13). Не публикуются ни при каких условиях `expiry` (R-U18) и `exchange.list` (R-U15.3) —
...
- **R-C2. Подстановка `${VAR}` и ссылки `env:VAR`.**
  ...
  - Значение, целиком имеющее вид `env:VAR`, — ссылка на секрет: допустима только в
    ...
    **Правило ведения каталога (не проверяется кодом, решение 121):** секретное значение хранится в
    ...
    `${VAR}`, всё равно не идёт нигде, кроме четырёх адресов R-U8.1 п. 6.
...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–123).**
  ...
  6. **Публикуемое значение.** Внутри блоков пп. 2 и 4 наружу идёт только строка, записанная в
     ...
     **Единственное исключение — четыре адреса**: `upstream.url`, `verify.url`, `exchange.url` и
     `exchange.revoke.url` публикуются **после** подстановки `${VAR}`, иначе прямого режима не
     существует (в действующем каталоге это `${TAG_MCP_URL}`, `${TAG_VERIFY_URL}` и `${TAG_API_BASE}`).
     Ссылка `env:VAR` в адресе запрещена схемой (решение 13, AC-15), поэтому адрес не несёт секрета
     по построению; секрет, записанный в адрес через `${VAR}`, — ошибка **ведения** каталога, прямо
     запрещённая правилом ведения в R-C2 и кодом не проверяемая (Hub не отличает значение переменной
     от адреса), поэтому теста на неё нет и быть не может. Состав `field` (`label`, `hint`, `docs_url`, `placeholder`, …) ревизией 4.4
     не затрагивается: он публиковался и до неё, `env:VAR` в нём запрещён схемой, и правило п. 6 на
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     **всего блока**, в котором лежит: блок отсутствует целиком, а не «без одного заголовка».
     ...
     `issues_permanent_token`, `field`) остаются на месте.
  8. **Единственный путь наружу.** Значения заголовков и тел попадают в публичное представление
     только через одну функцию, которая для набора возвращает либо все значения строками, либо
     «нельзя» (и тогда действует п. 7). Другого пути чтения этих значений в публичное представление
     нет: объекты-ссылки (`EnvRef`) и обёртки секретов в нём не разворачиваются никогда, и добавить
     позже частичную публикацию, не тронув это правило, невозможно.
...
| R-U8.1 п. 6 (публикуемое значение; исключение для четырёх адресов) | AC-252 |
| R-U8.1 п. 7 (только целиком; гранулярность снятия) | AC-252 |
| R-U8.1 п. 8 (единственный путь наружу) | AC-252, AC-14 |
| R-U8.1 п. 9 (след WARNING без значений и без имён переменных) | AC-252 |
...
Обязательные негативные сценарии ревизии 4.4: способ `oauth2` рядом с доступным `user_token` — ни
одного нового ключа (AC-185); карточка без `auth_methods` и карточка со всеми способами `oauth2` —
блока `upstream` нет (AC-22, AC-252); недоступный способ `user_token` — ни `verify`, ни `exchange`
(AC-236, AC-252); ссылка `env:VAR` в заголовке `verify`, `exchange`, `exchange.revoke`,
`credential_headers` и `static_headers` — соответствующий блок отсутствует целиком, а строк `env:`
и имени переменной нет нигде в ответе (AC-252); подстановка `${VAR}` в заголовке — блок отсутствует
целиком, а значение переменной в ответ не попадает (AC-252); `${VAR}` в адресе — блок публикуется,
адрес подставлен (AC-252); `expiry` и `exchange.list` — нет ни при каких условиях (AC-227, AC-252).
...
и блока `upstream`) — ни одно из прежних его утверждений не изменено и не ослаблено. Ни один другой
