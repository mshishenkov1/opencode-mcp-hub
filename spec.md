...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–123).**
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     **всего блока**, в котором лежит: блок отсутствует целиком, а не «без одного заголовка».
     ...
     собственный негативный прогон; фактическое состояние перечня приведено в §34.3.
...
| R-U8.1 п. 5 (`expiry` и `exchange.list` не публикуются никогда) | AC-227, AC-252 |
...
| R-U8.1 п. 9 (след WARNING без значений и без имён переменных) | AC-252 — пять записей на каждую загрузку; проверяется полный текст записи, включая поля `extra=`, а не только сообщение |
| R-U8.1 п. 10 (`available` не подменяется) | AC-252, AC-236 |
| R-U8.1 п. 11 (границы: well-known, `/ui/*`, собственный путь Hub) | AC-185, AC-252 |
...
- `expiry` и `exchange.list` — нет ни при каких условиях (AC-227, AC-252).

**Чего в этом перечне нет.** Три набора из перечня п. 7 собственного негативного прогона не имеют:
`exchange.headers`, `exchange.body` и `exchange.revoke.body`. Непубликуемое значение в них снимает
блок тем же кодом, что и в `exchange.revoke.headers` (карточка `subst`), но путь каждого набора
выводится из проверки отдельно и независимо, поэтому вывод любого из трёх сегодня не покраснеет ни
одним тестом — тот же класс дефекта, что был найден ревью на `static_headers`. Задание test-агенту
сформулировано в §34.5. До его закрытия §34.3 не вправе утверждать покрытие этих трёх наборов.
Не покрыты прогоном и покрыты быть не могут: второй рубеж п. 8 (недостижим, решение 124) и пять
...
`reports/review-rev44-1.json` (findings 1 и 2) состав AC-252 расширен с пяти карточек до **семи**
(добавлены `statichdr` и `twoways`, а у `clean` появились непустые дословные `static_headers`), и
ожидаемых записей следа стало **пять** вместо трёх; ни одно прежнее утверждение AC-252 не снято и
