...
> прежних ключей при повторном входе (по `key_alias`, значений прежних ключей у Hub нет), включается
> настройкой `HUB_LOGIN_REVOKES_PREVIOUS_KEYS`, **по умолчанию выключен**, чтобы дословно сохранить
> принятое поведение AC-45; рекомендуемая конфигурация — включённая. Отзыв опирается на реально
...
> AC-244…AC-249. Совместимость: новых обязательных переменных нет, миграций нет, ни одно правило и ни
> один прежний AC не изменены и не ослаблены — в частности AC-45 (повторный вход) и AC-50 (кэш
> аутентификации 60 с) сохраняются дословно. Внешняя зависимость: какие учётные данные принимает
...
- **R-K1. Загрузка настроек.** Настройки читаются через pydantic-settings из окружения с префиксом `HUB_`.
  ...
  | `HUB_LOGIN_REVOKES_PREVIOUS_KEYS` (ревизия 4.3) | `false` → повторный вход прежние ключи не отзывает (R-L5, R-L12) |
  | `HUB_LOG_LEVEL` | `INFO` |
...
- **R-L5. Сохранение.** Перед ответом `ready` Hub в одной транзакции: upsert `users` по `user_id`
  ...
  **Дополнение ревизии 4.3:** последняя фраза остаётся поведением по умолчанию (AC-45 дословно) и
  меняется только при включённой настройке `HUB_LOGIN_REVOKES_PREVIOUS_KEYS` — тогда после сохранения
  нового ключа прежние постоянные ключи того же пользователя отзываются по R-L12. Отсутствие значений
...
- **R-L12. Отзыв прежних ключей при повторном входе (решения 113, 114).**
  ...
  1. **Настройка.** `HUB_LOGIN_REVOKES_PREVIOUS_KEYS` — bool, по умолчанию **`false`**: поведение
     R-L5 и AC-45 дословно (повторный вход добавляет ключ, прежние остаются валидными). При `true`
     работают пп. 2–6. Рекомендуемая конфигурация — `true`: именно она закрывает заказчикову
     «перелогин не плодит ключи» и делает выход необязательным ритуалом. Умолчание оставлено
     выключенным сознательно (решение 113).
...
112. Успех выхода определяется **локальной уборкой**, а не ответом LiteLLM: отказ отзыва честно виден
    ...
    на руках.
113. Отзыв прежних ключей при повторном входе включается настройкой и **по умолчанию выключен**:
    включённым по умолчанию он изменил бы поведение, зафиксированное принятым AC-45 («повторный вход
    добавляет ключ, старые остаются валидными»), и без спроса обесценивал бы ключ на втором устройстве
    пользователя. Рекомендуемая конфигурация — `true`; смена умолчания — решение заказчика и
    отдельная ревизия, в которой AC-45 придётся переписать (spec-агент не отменяет принятые критерии
    самовольно).
...
Критерии ревизии 4.3 (AC-244…AC-249) добавлены к существующим; ни один прежний AC не удалён, не
изменён и не ослаблен — в частности сохраняют силу **дословно** AC-45 (повторный вход при выключенной
настройке добавляет ключ, прежние остаются валидными), AC-50 (положительный результат аутентификации
живёт в кэше 60 с, если строку `api_keys` удалили в обход Hub), AC-44 (ключ хранится только хешем),
AC-48 и AC-49 (аутентификация Bearer) и AC-133 (веб-вход в режиме `litellm`).
