...
- сверх всех перечисленных случаев — обход опубликованного представления эталонной карточки
  ...
  загрузкой; список случаев здесь не выписывается намеренно — его порождает сам прогон.
...
любого из четырёх путей из проверки теперь краснеет (проверено инъекциями с оракулом), и краснеет
им же любое **пятое** значение, которое появится завтра.
