...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–127).**
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     ...
     **одного экземпляра** эталона, а условие, ложное на этом экземпляре, для обхода не существует.
     ...
     механизма ни то, ни другое не требует. Отдельно от него остаётся второй вход того же класса:
     проверка дословности обходит наборы карточки по собственному перечню имён
     (`credential_headers`, `static_headers`), и набор, добавленный к публикации мимо этого
     перечня, не проходит через множества пропуска вовсе — ни сверка 1, ни её счёт верхних вызовов
     его не видят (правка dev в §34.5 закрывает и его). Против обоих входов у правила прогона нет —
     только обязанность ниже, и она названа прямо, а не подразумевается. Отсюда
...
129. **Обещание опирается на то, что подаётся в код, а не на то, что рядом с ним лежит; рейтинг
    ...
    **Что осталось открытым и названо остатком, а не покрытием:** условный ключ, источник
    которого — **новое** поле `ServerModel`, которого эталонная карточка не касается, и набор
    карточки, добавленный к публикации мимо перечня имён внутри самой проверки дословности. Оба
    того же жанра, оба закрываются без нового механизма (внесением поля в эталон; выносом перечня
    имён в константу — задание dev в §34.5), и ни для одного из двух прогона сегодня нет.
