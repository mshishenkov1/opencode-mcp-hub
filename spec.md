...
Что это даёт. Новое публикуемое строковое значение в блоках пп. 2 и 4 попадает в обход
**автоматически** и обязано либо снимать блок, либо быть внесённым в литерал исключений — то есть
пятое повторение либо краснеет само, либо становится видимой строкой в диффе прогона, которую
ревью не пропустит. Сужение проверки в коде (любой новый путь в кортежах пропуска) краснеет тем же
прогоном в тот же день. Заодно сторож покрыл и четыре описательных значения из задания выше:
...
остаётся на именованных карточках AC-252 (см. выше, «что при этом потеряно»). Более слабая замена —
сверка кортежей пропуска с литералом прямо в прогоне — ловит сужение проверки, но **не** ловит
появление нового публикуемого поля, то есть ровно тот случай, которым все четыре повторения и
начинались; к ней прибегать не пришлось, обход написан.
