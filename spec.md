...
- **R-U16. Что видит пользователь (дополнение R-U8).**
  ...
  - Публичное представление способа подключения (`/api/catalog`, R-U8) дополняется полем
    ...
    появляется (AC-22 не затронут). **Уточнение ревизии 4.1 (решение 103):** признак вычисляется по
    ...
    такого способа наружу по-прежнему не отдаётся и не читается — R-U12), и оговорка R-U12 о
    недоступных способах на этот булев признак не распространяется.
...
- **R-C7. Сквозные поля карточки: `permission_groups` и `type` (решения 104–108).** Hub протаскивает
  ...
  3. **Отсутствие полей — прежнее поведение байт в байт.** Если у сервера не заданы ни
     ...
     AC-22 и AC-185 сохраняют силу дословно.
