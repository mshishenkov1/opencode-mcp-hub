...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–127).**
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     **всего блока**, в котором лежит: блок отсутствует целиком, а не «без одного заголовка».
     ...
     на счёте, переписанном в текст, а на сверке с опубликованным представлением (ниже). Эти четыре — описательные поля обмена (имя поля в ответе целевой
     ...
     **Перечень, который ведут руками, защитой не является.** Четыре пробела подряд —
     ...
     ожиданием: блок опубликован, значение развёрнуто. Тогда новое публикуемое строковое значение
     попадает под проверку в тот же день, когда появляется: оно либо снимает блок, как требует этот
     пункт, либо становится видимой строкой в перечне исключений — молча мимо проверки оно не
     проходит; сужение самой проверки в коде (новый пропускаемый путь) краснеет тем же прогоном.
