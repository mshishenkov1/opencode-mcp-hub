...
- **R-K3. Секреты не утекают.** `HUB_SECRET_KEY`, `HUB_ENCRYPTION_KEY`, `HUB_ADMIN_TOKEN`, ключи LiteLLM
  (`sk-…`, JWT), poll-секреты (Hub и LiteLLM), `client_secret`, значения `env:VAR` никогда не попадают в
  логи, `audit_log.details`, ответы `/health`, `/api/*`, `/.well-known/opencode`, `/remote-config`, `/metrics`.
...
- **R-C1. Схема.** Каталог — YAML c обязательными полями `version` (целое ≥ 1) и `servers` (список, может
  ...
  |---|---|---|
  | `alias` | обяз. | `^[a-z][a-z0-9-]{1,31}$`, уникален в файле |
...
- **R-C2. Подстановка `${VAR}` и ссылки `env:VAR`.**
  ...
  - Значение, целиком имеющее вид `env:VAR`, — ссылка на секрет: допустима только в
    ...
    префиксом `env:`, ни значение не сериализуются.
...
- **R-L9. Скрытие внутренних данных.** Ответы `/cli/*` никогда не содержат `poll_secret` и `login_id`
  LiteLLM; поллинг LiteLLM выполняется только Hub'ом.
