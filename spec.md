...
- **R-U3. Проверка токена перед сохранением (`verify`).** Подполя:
  ...
  - код ответа равен `expect_status` (или, если `expect_status` не задан, любой 2xx) → токен принят;
  ...
  - `account_field` отсутствует в JSON-ответе — не ошибка: `provider_account` остаётся прежним/пустым.

  Проверка выполняется **до** любой записи в БД и KV. Тело ответа целевой системы целиком никуда не
  логируется: в журнал попадают alias, идентификатор способа и HTTP-код.
