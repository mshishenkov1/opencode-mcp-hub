...
| R-U8.1 п. 6 (публикуемое значение; пять непроверяемых входов) | AC-252 — дословность и снятие; AC-15 — `env:VAR` вне разрешённых полей отвергается загрузкой. Сами пять исключений (четыре адреса и `field`) кодом не проверяются: теста на них нет и быть не может (см. п. 6 и решение 125) |
...
абзаца оказались завышенными):

- способ `oauth2` рядом с доступным `user_token` — ни одного нового ключа (AC-185);
- карточка без `auth_methods` и карточка со всеми способами `oauth2` — блока `upstream` нет
  (AC-22, AC-185; в AC-252 карточек с `oauth2` нет);
