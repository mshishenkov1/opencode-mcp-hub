...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–127).**
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     ...
     **Полнота обхода равна полноте эталонного экземпляра — одним обходом граница не держится.**
     ...
     и новый пропускаемый путь — либо краснеют, либо становятся видимой строкой в диффе прогона.
     Чего она **не** даёт и что здесь названо прямо: у блока `upstream` модели нет — его состав
     складывает сама публикация, — поэтому сверять там можно только опубликованный состав с тройкой
     п. 4, а такая сверка видит новый ключ лишь тогда, когда он публикуется **безусловно**. Ключ,
     который публикация складывает сама и отдаёт только при заданном значении, не виден ни одной из
     трёх сверок; проверка дословности к тому же обходит наборы карточки по собственному перечню
     имён (`credential_headers`, `static_headers`), и набор, добавленный мимо этого перечня, не
     проходит через множества пропуска вовсе. Против этого входа у правила нет прогона — только
     обязанность ниже, и она названа прямо, а не подразумевается. Отсюда
