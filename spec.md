...
> **пять**, и это исчерпывающий перечень входов, дословность которых код не проверяет: четыре
...
> подобъект `field` способа, публикуемый всегда и целиком (решение 125). Не публикуются ни при каких условиях `expiry` (R-U18) и `exchange.list` (R-U15.3) —
...
- **R-C2. Подстановка `${VAR}` и ссылки `env:VAR`.**
  ...
  - Значение, целиком имеющее вид `env:VAR`, — ссылка на секрет: допустима только в
    ...
    **Правило ведения каталога (не проверяется кодом, решение 121):** секретное значение хранится в
    ...
    каталога (MR и CI). Публикация от этого не зависит: наружу значение, полученное подстановкой
    `${VAR}`, всё равно не идёт нигде, кроме пяти непроверяемых входов R-U8.1 п. 6 — четырёх
    адресов и подобъекта `field` (там оно публикуется **развёрнутым**, и ответственность за это
    несёт то же правило ведения).
