...
- у блока `upstream` модели нет — его состав складывает публикация, поэтому для него сверяется
  ...
  `static_headers`. Эта половина сверки слабее двух других, и задание этого не скрывает: она ловит
  новый ключ, публикуемый **безусловно**, и эталонную карточку, переставшую задавать `static_headers`,
  но не ловит ключ, отдаваемый только при заданном значении, — модели, по которой его можно было бы
  увидеть до публикации, у блока нет. Против этого входа остаётся обязанность п. 7 вести эталонную
  карточку исчерпывающей.
