...
- **R-L12. Отзыв прежних ключей при повторном входе (решения 113, 114).**
  ...
  3. **Чем отзывается.** `POST {LITELLM}/key/delete` с телом `{"key_aliases": [<алиасы прежних
     ...
     потерянное условие по `user_id` — без лимита стоила бы ключей всем пользователям сразу).
     ...
     **Точность обещания (решение 119):** лимит считает **алиасы в запросе**, то есть ограничивает
     число строк `api_keys` и, следовательно, число пользователей, задетых при ошибке фильтра;
     сколько ключей LiteLLM висит под одним алиасом, Hub не наблюдает (списка ключей по алиасу у
     него нет — см. ниже) и не ограничивает. Читать лимит как «не более двадцати отозванных ключей
     LiteLLM» нельзя: страховка — от массового отзыва по чужим строкам, а не от множественности на
     стороне LiteLLM, которая Hub'у недоступна ни для наблюдения, ни для ограничения.
...
119. **Алиас только что выданного ключа не отзывается никогда** — принят вариант (а) диспута
    ...
    алиасы, а не ключи LiteLLM, и множественность ключей под одним алиасом Hub не наблюдает.
