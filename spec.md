...
негативный прогон теперь имеют все **семь** наборов заголовков и тел (§34.3), а текст AC-252
приведён к факту: одиннадцать карточек и десять записей следа.

**Test-агенту — четыре описательных значения без прогона (ревью `reports/review-rev44-3.json`,
major; ОТКРЫТО).** Перечень п. 7 приведён к перечню публикуемого и содержит теперь одиннадцать
входов; у четырёх из них — `verify.account_field`, `exchange.description`, `exchange.token_field`,
`exchange.token_id_field` — нет ни одного негативного прогона, хотя код проверяет их наравне с
заголовками, а вывод их из проверки не краснеет ничем (§34.3). Задание — четыре карточки AC-252,
каждая кладёт непубликуемое значение **единственно** в своё значение, всё остальное в карточке
дословно (иначе прогон не различает пути и пробел остаётся):

- `acctfield` — подстановка `${VAR}` в `verify.account_field`: блока `verify` нет целиком, `exchange`
  способа и `upstream` карточки на месте, у способа прежние `available: true`, `id`, `title`,
  `field`, `issues_permanent_token`; ровно одна запись WARNING на загрузку — `alias` + `verify`;
- `exchdesc` — подстановка `${VAR}` в `exchange.description`: блока `exchange` нет целиком вместе с
  вложенным `revoke`, `verify` и `upstream` на месте; одна запись — `alias` + `exchange`. Маркер
  обязан быть **коротким**: `description` ограничена ста символами, а схема видит значение уже
  подставленным — иначе прогон покраснеет ошибкой загрузки, а не тем, ради чего написан;
- `exchtok` — то же в `exchange.token_field`; `exchtokid` — то же в `exchange.token_id_field`:
  снятие такое же, по одной записи `alias` + `exchange` на карточку.

Значение каждой переменной — уникальный маркер утечки, которого не должно быть нигде в теле
`GET /api/catalog`. На верной реализации (HEAD 608df72) все четыре прогона зелёные: проверка
дословности обходит весь сырой блок, и ни один из четырёх путей не выведен из-под неё. Ложного
красного здесь нет и быть не может — снятие блока по непубликуемому значению есть само правило
п. 7, а не догадка о реализации. Числа AC-252 (карточки и записи следа) после появления карточек
приводит к факту spec-агент — тем же порядком, каким это делалось в прошлых кругах; до тех пор
текст AC-252 верно описывает одиннадцать карточек и десять записей.

**Test-агенту — положительный прогон на `exchange.list` (minor, ревью
`reports/review-rev44-3.json`; ОТКРЫТО).** Утверждение п. 5 «значения `exchange.list` на публикацию
`exchange` не влияют» сегодня не краснеет ни при каком изменении. Задание — одна карточка `listex`:
...
**Test-агенту — СТОРОЖ ПЕРЕЧНЯ: прогон, порождающий случаи из опубликованного представления
(ревью `reports/review-rev44-3.json`; ОТКРЫТО, приоритет высокий).** Четыре пробела подряд —
...
прогоном в тот же день. Заодно сторож покрывает и четыре описательных значения из задания выше: если
он написан, отдельные карточки `acctfield`, `exchdesc`, `exchtok`, `exchtokid` становятся
избыточными, и достаточно его одного. Прогон помечается маркером `@pytest.mark.ac("AC-252")` —
нового критерия он не требует (утверждение то же: граница держится целиком); если test-агент
предпочтёт отдельный критерий, его заводит spec-агент **вместе** с появлением прогона, иначе гейт
G6 покажет непокрытый AC.
...
не попадают; на них остаётся положительный прогон из задания выше. Не закрывает он и открытую часть
перечня п. 6 (`title`, `unavailable_reason` способа, поля карточки R-C6): их граница — правило
ведения каталога R-C2, а не R-U8.1, и требовать снятия блока там нельзя. Более слабая замена —
...
начинались; поэтому она годится только как временная мера, если обход опубликованного
представления окажется дороже, чем стоит.
...
- на AC-252 задание не влияет: ни в одной из его одиннадцати карточек подстановки в `field` нет,
  и десять записей следа остаются десятью. Если реализация всё же тронет этот счёт, значит, она
