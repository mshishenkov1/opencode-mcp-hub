...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–123).**
  ...
  6. **Публикуемое значение.** Внутри блоков пп. 2 и 4 наружу идёт только строка, записанная в
     ...
     **Исключений пять, и это исчерпывающий перечень публикуемых значений, дословность которых код
     не проверяет** (решения 123 и 125). Четыре из них — адреса: `upstream.url`, `verify.url`,
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     **всего блока**, в котором лежит: блок отсутствует целиком, а не «без одного заголовка».
     ...
     Перечень наборов, проверяемых на дословность, обязан **совпадать** с перечнем публикуемых по
     пп. 2 и 4: `credential_headers` и `static_headers` карточки, `verify.headers`,
     `exchange.headers`, `exchange.body`, `exchange.revoke.headers`, `exchange.revoke.body`.
     Публикуемое значение, выведенное из-под этой проверки, — дефект границы, а не умолчание: такая
     утечка не видна снаружи ничем, кроме самого секрета, поэтому каждому набору перечня полагается
     собственный негативный прогон. Все **семь** наборов перечня такой прогон имеют (AC-252,
     карточки `envverify`, `envcred`, `statichdr`, `exchhdr`, `exchbody`, `revokebody`, `subst`,
     `twoways` и `midstr`); фактическое состояние перечня приведено в §34.3.
