...
> AC-244…AC-250. По диспуту test-агента R-L12.3 переписан честно: остатка «на следующий вход» не
...
> (решение 118). Совместимость: новых обязательных переменных нет, миграций нет, ни одно правило не
...
- **R-L12. Отзыв прежних ключей при повторном входе (решения 113, 114).**
  ...
  1. **Настройка.** `HUB_LOGIN_REVOKES_PREVIOUS_KEYS` — bool, по умолчанию **`true`** (решение
     ...
     ключей не плодит и явный выход перестаёт быть обязательным ритуалом. Значение `false` возвращает
  ...
  2. **Что отзывается.** После **успешной** выдачи и сохранения нового постоянного ключа (R-L4,
     ...
     `HUB_KEY_ALIAS_PREFIX`, кроме только что созданной. Ключи `key_kind = "jwt"` не отзываются
     никогда — их Hub не выпускал (R-L4, запасная ветка), они истекают сами; это тот же принцип, что
  ...
  4. **Порядок и влияние на вход.** Сначала выпустить и сохранить новый ключ, потом отзывать
     ...
     **Дополнение ревизии 4.3 по диспуту (решение 118):** закрытый набор `outcome` расширен значением
     `"skipped"` — «алиас не попал в лимит п. 3, запрос отзыва по нему не отправлялся». Запись
  ...
  6. **Остаточный риск и след.** Ключ остаётся действительным в самом LiteLLM в двух случаях:
     (а) удалённый отзыв не удался; (б) алиас не попал в лимит 20 (п. 3). В обоих Hub, удалив строку,
     повторить попытку уже не может: колонку-пометку в `api_keys` ревизия не заводит и миграции не
  ...
  7. **Согласование с выходом.** После `DELETE /api/me/key` следующий вход отзывать нечего — строк
     ...
     инварианту: у пользователя не больше одного действующего постоянного ключа Hub'а на установку.
...
| R-L12.4, R-L12.6 (аудит `outcome: "skipped"` на каждый неотозванный алиас, остаточный риск) | AC-250 |
...
в аудит как пропущенные — AC-250.

Критерии ревизии 4.3 (AC-244…AC-250) добавлены к существующим. Прежних критериев не удалено ни
...
отзыва ушёл по алиасу). Дословно сохраняют силу AC-50 (положительный результат аутентификации живёт в
