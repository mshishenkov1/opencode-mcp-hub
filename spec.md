...
> AC-244…AC-249. Совместимость: новых обязательных переменных нет, миграций нет, ни одно правило не
...
- **R-L12. Отзыв прежних ключей при повторном входе (решения 113, 114).**
  ...
  3. **Чем отзывается.** `POST {LITELLM}/key/delete` с телом `{"key_aliases": [<алиасы прежних
     ...
     `/key/generate`, — та же привилегия, никакой новой не требуется. За один вход — не более одного
     запроса отзыва и не более 20 алиасов в нём (страховка от массового отзыва при ошибке выборки, по
     образцу R-U15.2); остаток убирается следующим входом.
  4. **Порядок и влияние на вход.** Сначала выпустить и сохранить новый ключ, потом отзывать
     ...
     `details {key_alias, reason: "relogin", outcome: "ok"|"failed"}`, без значений ключей.
  ...
  6. **Остаточный риск и след.** Если удалённый отзыв не удался, ключ остаётся действительным в самом
     LiteLLM, а Hub, удалив строку, повторить попытку уже не может: колонку-пометку в `api_keys`
     ревизия не заводит и миграции не вводит. Риск принят и ограничен: (1) алиас детерминирован
     (R-L4) и записан в аудит (`login_completed`, `key_revoked` с `outcome: "failed"`), поэтому
     администратор LiteLLM может убрать ключ по алиасу — след не потерян; (2) прежний ключ сразу
     перестаёт открывать Hub, то есть доступ к коннекторам и целевым системам закрыт; (3) остаётся
     ...
     от прав пользователя в LiteLLM.
...
Критерии ревизии 4.3 (AC-244…AC-249) добавлены к существующим. Прежних критериев не удалено ни
