...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–123).**
  ...
  6. **Публикуемое значение.** Внутри блоков пп. 2 и 4 наружу идёт только строка, записанная в
     ...
     **ведения** каталога, прямо запрещённая правилом ведения в R-C2 и кодом не проверяемая (Hub не
     ...
     публикуется **развёрнутым** — ровно как в адресах и ровно с тем же статусом: это ошибка ведения
     каталога (R-C2), кодом не проверяемая, теста на неё нет и быть не может. Правило «только
...
- на AC-252 задание не влияет: ни в одной из его одиннадцати карточек подстановки в `field` нет,
  и десять записей следа остаются десятью. Если реализация это изменит — изменился не тест, а
  правило, и виноват не счёт.
