...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–127).**
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     **всего блока**, в котором лежит: блок отсутствует целиком, а не «без одного заголовка».
     ...
     пп. 2 и 4 за вычетом пяти исключений п. 6. Входов **одиннадцать**: семь наборов «имя → строка»
     — `credential_headers` и `static_headers` карточки, `verify.headers`, `exchange.headers`,
     `exchange.body`, `exchange.revoke.headers`, `exchange.revoke.body` — и четыре отдельных
     строковых значения: `verify.account_field`, `exchange.description`, `exchange.token_field`,
     `exchange.token_id_field`. Эти четыре — описательные поля обмена (имя поля в ответе целевой
     ...
     полагается собственный негативный прогон — карточка, где непубликуемое значение лежит
     единственно в нём. Фактическое состояние перечня: собственный прогон имеют **семь** наборов
     заголовков и тел (AC-252, карточки `envverify`, `envcred`, `statichdr`, `exchhdr`, `exchbody`,
     `revokebody`, `subst`, `twoways` и `midstr`), а у четырёх описательных значений прогона **нет
     ни одного** — пробел назван ревью `reports/review-rev44-3.json`, задание test-агенту
     сформулировано в §34.5, и пока оно не исполнено, §34.3 обязана называть состояние как есть.
     В перечень **не входят** и три значения `method` (`verify.method`, `exchange.method`,
     ...
     опирается, и вывод `method` из-под проверки утечки не даёт; собственного негативного прогона
     эти три значения поэтому не требуют. По той же причине в перечне нет нестроковых значений
     ...
     которой `exchange` тем не менее опубликован (задание test-агенту, §34.5); сегодня его нет, и
     послабление в эту сторону не краснеет ничем.
