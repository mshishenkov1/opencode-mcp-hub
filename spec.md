...
- **R-U8.1. Граница публикации способов подключения (дополнение R-U8 и R-C6; решения 120–123).**
  ...
  7. **Только целиком (главное правило границы).** Непубликуемое значение (п. 6) снимает публикацию
     **всего блока**, в котором лежит: блок отсутствует целиком, а не «без одного заголовка».
     ...
     не имеет права оказаться снятым «не у того» способа — соответствие способов их значениям
     ведётся по `id`, а не по порядку в файле.
...
125. **`field` — пятый непроверяемый вход; это дефект текста, а не кода.** Ревью (finding 6)
    ...
    **развёрнутым**, тогда как п. 6 называл непроверяемыми входами только четыре адреса. Проверка
    ...
    с другой ценой — отказ в загрузке каталога, а не снятие блока.
...
**Test-агенту — три набора без негативного прогона (§34.3, «Чего в этом перечне нет»).** В AC-252
не хватает карточек, где единственное непубликуемое значение лежит в `exchange.headers`,
`exchange.body` и `exchange.revoke.body`. Проверка дословности ведётся по путям внутри блока, и путь
каждого набора выводится из неё отдельно: сегодня такой вывод не покраснеет ни одним тестом, хотя
выпускает наружу развёрнутое значение переменной — ровно тот дефект, который ревью нашло на
`static_headers` (finding 1). Ожидаемое поведение во всех трёх случаях одинаково и уже описано
правилом: блока `exchange` нет **целиком, вместе с вложенным `revoke`**, `verify` и `upstream` той
же карточки на месте, значения переменной нет ни в теле `/api/catalog`, ни в записи журнала, и на
каждую загрузку приходится ровно одна дополнительная запись WARNING (`alias` + `id` способа +
`exchange`). Для `exchange.headers` годится и `env:VAR` (схема его там допускает, R-C2), для тел —
только `${VAR}`: `env:VAR` в `exchange.body` и `exchange.revoke.body` схема отвергает при загрузке
(AC-15), и это отдельный, уже покрытый сценарий. Текст AC-252 после этого дополняется тремя
карточками и тремя записями следа — правка спеки, не теста.

**Dev-агенту заданий ревизия не порождает.** Оба оставшихся замечания ревью закрыты текстом
правила, а не кодом: второй рубеж п. 8 остаётся в коде без изменений (решение 124 — удалять его
нельзя, делать достижимым специально — тоже), а `field` назван пятым непроверяемым входом п. 6
(решение 125 — проверка кодом отклонена: `field` обязан присутствовать всегда, иначе способ
перестаёт подключаться и собственным путём Hub).
