...
124. **Второй рубеж границы — защита в глубину, а не проверяемый затвор; формулировка п. 8
    ...
    ревьюера (замена отказа на пропуск значения: 983 passed, зелено). Причина не в тесте, а в
...
127. **Четыре описательных значения обмена остаются под проверкой; сужать перечень нельзя.**
    ...
    границы** — вывод любого из четырёх путей из-под проверки не краснеет ни одним из 983 прогонов,
    и это ровно тот дефект, который выдавался blocker'ом в первом и втором кругах. Перечень п. 7
    приведён к перечню публикуемого (одиннадцать входов), §34.3 называет пробел как есть, прогоны
    заданы test-агенту (§34.5), и там же сформулирован сторож, после которого ручная сверка перечня
    с прогонами перестаёт быть единственной защитой от пятого повторения.
...
| R-U8.1 п. 5 (`expiry` и `exchange.list` не публикуются никогда) | AC-227 — единственный прогон: только у него карточка объявляет и `expiry`, и `exchange.list`, и он ловит их не по имени ключа, а по значениям-маркерам, встречающимся только внутри них. В AC-252 карточек с `expiry` и `list` нет вовсе, поэтому покрытием п. 5 он не является (сверка `list` там холостая) |
...
| R-U8.1 п. 7 (только целиком; гранулярность снятия; независимость способов) | AC-252 — снятие `verify`, снятие `exchange` вместе с вложенным `revoke`, снятие `upstream` и по `credential_headers`, и по `static_headers`; независимость двух доступных способов одной карточки — карточка `twoways`. Перечень п. 7 содержит **одиннадцать** входов; собственный негативный прогон имеют **семь** наборов заголовков и тел, а четыре описательных значения (`verify.account_field`, `exchange.description`, `exchange.token_field`, `exchange.token_id_field`) не покрыты **ни одним** прогоном, хотя код проверяет их наравне с заголовками: пробел найден ревью `reports/review-rev44-3.json` (инъекция N16 — вывод этих четырёх путей из проверки даёт полностью зелёный набор при подтверждённой оракулом утечке), задание test-агенту — в §34.5. Подробности ниже |
...
| R-U8.1 п. 9 (след WARNING без значений и без имён переменных) | AC-252 — **десять** записей на каждую загрузку (старт и `reload`), и ни одной на блоки, которые сняты не были; проверяется полный текст записи, включая поля `extra=`, а не только сообщение |
...
- `expiry` и `exchange.list` — нет ни при каких условиях (AC-227; в AC-252 карточек с `expiry` и `list` нет, и сверка `list` там холостая).
...
**Чего в этом перечне нет — 2 (открытый пробел, ревью `reports/review-rev44-3.json`).** Собственного
негативного прогона нет **ни одного** у четырёх описательных значений, которые публикуются и
которые код проверяет наравне с заголовками: `verify.account_field` (внутри блока `verify`),
`exchange.description`, `exchange.token_field` и `exchange.token_id_field` (внутри блока
`exchange`). Пробел того же класса, что два предыдущих, и найден он тем же способом: вывод этих
четырёх путей из проверки дословности не краснеет ни одним из 983 прогонов, тогда как оракул на тех
же путях показывает развёрнутое значение переменной в теле `GET /api/catalog`. От прежних двух он
отличается местом: три набора `exchange` были пропущены **перечнем** §34.3 при верном тексте
правила, а эти четыре были пропущены самим перечнем п. 7 при том, что п. 6 их называет прямо, —
поэтому правка первична в правиле (сделана: перечень п. 7 теперь одиннадцать входов), а прогоны
формулируются заданием test-агенту (§34.5). До исполнения задания граница этих четырёх значений
держится только на коде и не удерживается ни одним прогоном; поведения это не меняет — на чистом
HEAD утечки нет.

**Не покрыт прогоном и skip-путь `('list',)`** — утверждение п. 5 «значения `exchange.list` на
публикацию блока `exchange` не влияют» не краснеет ни при каком изменении: снятие послабления
делает код строже и утечки не даёт (ревью `reports/review-rev44-3.json`, инъекция N18). Ему
полагается **положительный** прогон, задание — в §34.5.
...
собственного негативного прогона и три публикуемые строки `method` (`verify`, `exchange`,
`exchange.revoke`) и нестроковые `expect_status` и `require_account`: их значения ограничены схемой
(три литерала, число, флаг), непубликуемое значение через них наружу не выходит ни при каком
каталоге, и вывод их из-под проверки утечки не даёт — сужение перечня п. 7 здесь сделано **с
указанием причины** (сплошная сверка перечня с составом публикуемого, этот круг правок).
...
`midstr`. Итог, сверенный с зелёным прогоном `tests/test_user_token.py`, а не с замыслом:
**одиннадцать** карточек (`clean`, `envverify`, `envcred`, `subst`, `statichdr`, `closed`,
`twoways`, `exchhdr`, `exchbody`, `revokebody`, `midstr`) и **десять** ожидаемых записей следа на
каждую загрузку вместо прежних трёх. Текст критерия (`given` и `then`) приведён к этим числам в
этом же круге правок — расхождение текста AC-252 с тестом, отмеченное ревью
(`reports/review-rev44-2.json`, finding 3), устранено; ни одно прежнее утверждение AC-252 не снято и
не ослаблено — все дополнения аддитивны. Ни один другой
