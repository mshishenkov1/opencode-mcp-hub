...
## 10. После review-i5-3

Вход: `reports/review-i5-3.json` (вердикт `request_changes`) — блокер по `purge_paths` и два
`major` с `assignee: test` (находки 2 и 3). DEV закрыл блокер системно (`0b45b54`, `485efd1`,
`09e69e8`, `4e4c236`), spec-agent внёс ревизию 1.4 (`651e044`: правило **N5-P9**, ужесточённый
**N5-R2**, критерии **AC-143** и **AC-144**). Ниже — что закрыто со стороны тестов.

### 10.1 Что добавлено

| Файл | Тестов | Чем закрывает находку |
|---|---|---|
| `installers/tests/bats/37-path-normalization.bats` (новый) | 24 | находка 2: формы `purge_paths`, схлопывающиеся в сам `$HOME`; таблица `path_normalize`/`path_is_inside`/`path_require_child`; defense in depth на `purge_each`; нормализация `--prefix` и `XDG_CONFIG_HOME` |
| `installers/tests/bats/90-release.bats` (усилен) | 29 (было 24) | находка 3: побайтовое сравнение значения `hub_url` после `json.load`; поведение при провале самопроверки |
| `installers/tests/bats/36-desktop-safety.bats` (усилен) | 24 (было 22) | находка 4 (minor): `is_safe_app_name` и `desktop_target_path` отвергают пробельное `app_name` — паритет с `Test-AppName` проверяется, а не читается из комментария |
| `installers/tests/pester/PathSafety.Tests.ps1` (новый) | 15 | Windows-паритет: `ConvertTo-NormalPath`, `Test-PathInside`, `Test-PurgePathSafe` |

Набор `bats`: **196 → 227** тестов. Набор Pester: **80 → 95** (в CI).

**Находка 2 (`70-uninstall.bats:133`, негативные `purge_paths` без нормализующихся форм) —
закрыта.** Опасные значения проверяются каждой формой из AC-143: `${HOME}//`, `~//`, `${HOME}///`,
`${HOME}/./`, `${HOME}/.`, `${HOME}/sub/../`, `${HOME}`, `~`, `/etc`, `~/../other`, относительные
`relative/opencode` и `.config/opencode`. По каждой форме проверяется: код выхода 2, предупреждение
с ИСХОДНЫМ значением, строка «Список purge_paths отвергнут целиком», домашний каталог существует,
обе канарейки (`$HOME/canary.txt` и `$HOME/Documents/very-important/data.txt`) на месте и совпадают
по sha256, снимок `$HOME` до и после запуска побайтово одинаков, ловушка `sudo` пуста. Отдельно —
список из двух записей (штатной и опасной): опасная отвергает список целиком, соседний штатный
каталог цел. Контроль: `${XDG_CONFIG_HOME}/opencode` удаляется как прежде, а «шумная» форма внутри
`$HOME` (`${HOME}//.local//share/./opencode`) нормализуется и печатается в выводе канонически.

**Defense in depth.** `purge_each` вызывается напрямую при `OPENCODE_INSTALLER_SOURCE_ONLY=1` в
обход `validate_purge_paths`: `mf_val` подменён источником значения, `rm` — журналирующей
заглушкой. На всех десяти небезопасных формах — код 2 и пустой журнал операций; на штатном пути
`rm -rf` вызывается ровно по нормализованному значению, а не по исходной строке.

**Нормализация `--prefix` и `XDG_CONFIG_HOME`.** Относительный `--prefix rel-opt` достраивается от
текущего каталога и устанавливает пакет в `<cwd>/rel-opt/bin`; `--prefix ../escape` и
`XDG_CONFIG_HOME=$HOME/../escape` отвергаются кодом 2 сообщением «Небезопасный путь (…)» до единой
файловой операции; схлопывающиеся формы (`<prefix>//`, `<prefix>/./`, `$HOME//cfg`) работают и
печатаются нормализованными.

**Находка 3 (`90-release.bats:349`, AC-142 проверял только валидность JSON) — закрыта.** Проверка
велась подстановкой команд, которая срезает завершающие переводы строк; теперь исходное значение
пишется в файл, а сравнение выполняется внутри `python3` (`io.open(..., newline='')` — иначе
текстовый режим переводит `\r` в `\n` и тест перестал бы различать эти символы). Значения:
перевод строки, табуляция, `\r`, `0x01`, `\b`/`\f`, двойная кавычка с обратным слэшем, кириллица и
«все классы разом». По каждому — либо код 2 без каталога `--out` (исход, разрешённый AC-142), либо
валидный JSON И побайтовое совпадение `hub_url` после `json.load`.

**Провал самопроверки (AC-144).** Наблюдается на КОПИИ `release.sh` с тождественным `json_escape`
(сам продуктовый файл не меняется): копия лежит в собственном корне, каталоги `common`, `linux`,
`macos`, `windows` подставлены симлинками, отличается ровно одна функция. Результат: код 1,
сообщение о невалидном манифесте, строка «Архив не прошёл самопроверку и удалён из каталога
сборки», архива и `SHA256SUMS` в `--out` нет. Контрольный тест собирает тем же способом копию с
исправным `json_escape` — код 0 и оба файла на месте, поэтому код 1 нельзя списать на способ
запуска копии.

### 10.2 Проверка тестов мутациями

Каждая мутация вносилась в продуктовый код, прогонялся полный `bats installers/tests/bats`, затем
`git checkout -- installers/common installers/release.sh installers/windows`.

| # | Мутация | Падений до правок | Падений после |
|---|---|---|---|
| M9 | `purge_path_resolve` печатает раскрытое значение без `path_normalize` — точное воспроизведение блокера ревью | **0** | **9** (AC-89, AC-91, AC-143) |
| M10 | Из `purge_each` снята повторная проверка `purge_path_is_safe` (defense in depth) | 0 | **2** (AC-143, прямой вызов `purge_each`) |
| M11 | `path_normalize` больше не отвергает сегмент `..` | 0 | **4** (AC-135, AC-143: таблица, `path_require_child`, `--prefix`, `XDG_CONFIG_HOME`) |
| M12 | Из `is_safe_app_name` снята проверка «непусто после отбрасывания пробельных символов» | 0 | **2** (AC-138) |
| M13 | `selfcheck_fail` не удаляет битый архив | 0 | **2** (AC-144 + `shellcheck` на осиротевшей переменной) |
| M14 | Из `path_is_inside` снят запрет `child == parent` | 0 | **0 — эквивалентный мутант** |
| M14b | В `path_is_inside` граница сегмента `"$parent"/?*` заменена простым префиксом `"$parent"*` | 0 | **1** (AC-135, AC-143: сосед с общим префиксом) |
| M14c | Обе проверки `path_is_inside` сняты одновременно | 0 | **12** (AC-91, AC-135, AC-143) |
| M15 | `path_require_child` больше не проверяет вложенность цели в свой каталог | 0 | **1** (AC-135, AC-143) |
| M4 | `json_escape` без замены `$'\n'` (мутация из `mutation_log` ревью, находка 3) | **0** | **2** (AC-142, AC-144) |

**M14 — эквивалентный мутант, и это содержательный факт, а не пробел набора.** Строка
`[ "$child" != "$parent" ] || return 1` в `path_is_inside` избыточна при живом шаблоне
`"$parent"/?*`: нормализованный путь не имеет завершающего разделителя, поэтому равный родителю
`child` шаблону не соответствует и функция и без этой строки возвращает 1. Наблюдаемого поведения,
различающего мутанта и оригинал, не существует — тест, который его «поймал» бы, проверял бы
внутренности реализации. Обратное тоже верно: при живой строке равенства мутация шаблона (M14b)
опасна только для соседа с общим префиксом (`/home/u` против `/home/user2`), и ловит её именно
модульная таблица. Совместно (M14c) обе снятые проверки дают 12 падений, включая сквозные прогоны
с удалением домашнего каталога, — то есть защита покрыта не одним тестом, а слоями.

`git status` по `installers/` после серии мутаций чист, изменены только файлы `installers/tests/`.

### 10.3 Маркеры AC и покрытие

Новые проверки размечены существующими критериями плюс введёнными ревизией 1.4:

| AC | Что проверяют новые тесты | Тестов (по `trace-ac.sh`) |
|---|---|---|
| AC-135 | таблица нормализации, строгая вложенность, `path_require_child` | 34 |
| AC-138 | пробельное `app_name` отвергается обеими линиями защиты | 26 |
| AC-89, AC-90, AC-91 | штатное удаление, пути вне `$HOME`, сам домашний каталог | — (в составе AC-143) |
| AC-142 | манифест валиден по RFC 8259 | 12 |
| **AC-143** | единая нормализация путей, строгая вложенность `purge_paths`, defense in depth, `--prefix`/`XDG_CONFIG_HOME` | 39 |
| **AC-144** | значение из манифеста не искажается; битый архив удаляется | 6 |

Формулировки AC-143 и AC-144, внесённые spec-agent в ревизии 1.4, тестам соответствуют: набор
покрывает каждый перечисленный в них случай, включая канарейки `$HOME/canary.txt` и
`$HOME/Documents/very-important/data.txt`, относительный путь `.config/opencode`, обойдённую
валидацию и строку об удалённом архиве. Расхождений между текстом критериев и набором нет.

### 10.4 Прогон и триаж

| Прогон | Команда | Результат |
|---|---|---|
| 1 | `bats installers/tests/bats` | **227 тестов: 227 ok, 0 not ok** |
| 2 | `bats installers/tests/bats` | **227 тестов: 227 ok, 0 not ok** — результаты совпадают построчно |
| — | `shellcheck -s bash` по `installers/**/*.sh` | 0 замечаний |
| — | `shellcheck -s bash` по собственным хелперам новых `.bats` | чисто; единственные подавления — `SC2016` и `SC2088` с обоснованием (`${HOME}` и `~` здесь не подстановка shell, а проверяемое значение, которое разворачивает установщик) |
| — | `installers/tests/trace-ac.sh` | код 0, 144 критерия, 128 с тестами, все `unit`/`integration` покрыты |

Падений нет, новых багов не заведено: поведение установщика после DEV-фиксов соответствует AC-143 и
AC-144 на всех проверенных значениях. Флаки не обнаружены — два полных прогона дали совпадающий
список результатов.

### 10.5 Ограничения

1. **Pester не выполнен локально: `pwsh` в среде отсутствует.** `PathSafety.Tests.ps1` (15 тестов)
   написан по чтению `install.ps1` и рассчитан на задание `pester` в `installers/ci/installers.yml`.
2. **Формы с обратным слэшем содержательно проверяются только на Windows-раннере.** Задание
   `pester` выполняется на `ubuntu-latest`, где `\` — обычный символ имени файла, а схлопывание
   `%USERPROFILE%\.` делает Windows-реализация `[IO.Path]::GetFullPath`. Поэтому таблица форм
   строится на РОДНОМ разделителе платформы (проверка содержательна на обеих), а буквальные
   варианты с `\` вынесены в отдельные `It`, которые на не-Windows помечаются `Set-ItResult
   -Skipped` — вхолостую «зелёными» они не проходят. Следствие: **на текущей конфигурации CI
   Windows-ветка требований AC-143 автоматикой не подтверждается**; чтобы закрыть это по-настоящему,
   в задание `pester` нужна матрица с `windows-latest`. Продуктовой ошибки здесь нет — на Windows
   `GetFullPath` схлопывает все перечисленные формы, — но это дыра в верификации, а не в коде.
3. Проверки, не зависящие от разделителя (сегмент `..` в сыром и раскрытом значении, относительный
   путь, путь вне профиля, `C:\Windows`, сосед с общим префиксом, пустые аргументы), выполняются на
   обеих платформах и содержательны уже сейчас.
