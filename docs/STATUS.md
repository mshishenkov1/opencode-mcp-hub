...
## Состояние CI/CD (проверено 2026-08-20) — требует внимания в первую очередь
| Репозиторий | Workflow | Триггеры | Что проверяет |
|---|---|---|---|
| `opencode-mcp-hub` | `.github/workflows/gates.yml` | PR, push в `pipeline/**` | гейты G1–G6 (тесты, coverage-diff, mutation, линт/типы, ревью, трассировка AC) |
| `opencode` (форк) | `.github/workflows/corp-ci.yml` | PR в `main`, теги `v*-magnit.*` | typecheck, тесты пакетов, дымовая проверка, сборка CLI ×3 ОС; по тегу — Desktop и черновик релиза |
| `magnit-tag-mcp` | `.github/workflows/ci.yml` | push в `main`, PR | линт, типы, тесты (порог покрытия 100 %) |

**Дыры (все цифры тестов в отчётах получены ЛОКАЛЬНО, прогоны в GitHub Actions ни разу не подтверждены):**
1. Не проверена история прогонов — `gh` в системе не авторизован (`gh auth login`). Сделать это первым делом.
2. **Установщики вне CI**: workflow лежит эталоном в `installers/ci/installers.yml` и не скопирован в `.github/workflows/`
   (агентам запрещено трогать `.github/`). 227 bats + 95 Pester в GitHub не гоняются. Копирование — ручное действие.
3. **Windows-ветка не проверяется**: Pester гоняется на ubuntu, где семантика путей другая; матрица с `windows-latest`
   была поставлена в работу, но задача оборвалась и не доделана. Часть требований AC-143/136/139/140 автоматикой не подтверждена.
4. **Desktop не собирается на PR** (только по тегу) — именно поэтому BUG-I4-002 и BUG-I4-003 нашлись вручную, а не в пайплайне.
5. **Mutation-гейт только в Hub**; в форке и ТЭГ-MCP его нет.
6. **CD отсутствует**: есть черновик релиза по тегу в форке; выкладка Hub (образ → реестр → Helm) не автоматизирована —
   по плану это платформа AI Lab, когда дадут namespace.

**План по CI на ближайшую сессию:** авторизовать `gh` и посмотреть реальные прогоны → скопировать workflow установщиков
в `.github/workflows/` → добавить матрицу `windows-latest` для Pester → добавить сборку Desktop в CI на PR →
решить по mutation-гейту для форка.
